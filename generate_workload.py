...
def generate_workload(num_user_attrs=5, num_rules=10, users=None, table_name="my_table"):
    users = users or ["alice", "bob", "charlie", "david", "eve"]
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python generate_workload.py <num_user_attributes> <num_abac_rules>")
        sys.exit(1)

    num_user_attrs = int(sys.argv[1])
    num_rules = int(sys.argv[2])

    script = generate_workload(num_user_attrs, num_rules)
    with open("generated_workload.sql", "w") as f:
        f.write(script)

    print(f"Generated workload with {num_user_attrs} user attributes and {num_rules} ABAC rules in 'generated_workload.sql'")
